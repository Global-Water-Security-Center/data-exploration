...
def main():
    ...
    parser = argparse.ArgumentParser(description=(
        'Calculate rain events from CIMP5 data from GEE. Generates a CSV '
        'table that whose rows are days and columns are daily mean precip '
        'over the given country.'))
    parser.add_argument(
    ...
    local_working_dir = tempfile.mkdtemp()
    local_shapefile_path = os.path.join(
        local_working_dir, '_local_ok_to_delete.shp')
    ...
    print(
        'waiting for GEE to compute, if the date range is large this '
        'may take a while')
